      ...
      <div className="h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-gradient-to-br from-[#8cc63f]/10 to-[#8cc63f]/20">
        {station.images.length > 0 ? (
          ...
          <div className="flex h-full items-center justify-center">
            <Zap className="h-6 w-6 text-[#8cc63f]/50" />
          </div>
        )}
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-sg font-semibold text-gray-900 group-hover:text-[#133c1d] transition-colors">
          {station.name}
        </p>
        ...
        <div className="mt-1.5 flex items-center justify-between">
          ...
          {station.distanceKm != null && (
            <span className="rounded-full bg-[#8cc63f]/10 px-2 py-0.5 text-[10px] font-semibold text-[#8cc63f]">
  ...
  return (
    <div className="w-52 space-y-2 py-0.5">
      ...
      <div className="space-y-1.5">
        <p className="font-sg font-semibold text-gray-900 text-sm leading-tight">{station.name}</p>
        ...
        <div className="flex items-center justify-between">
          ...
          {station.isVerified && (
            <span className="flex items-center gap-0.5 text-xs font-medium text-[#8cc63f]">
        ...
        <aside className={cn(
          ...
          <div className="shrink-0 bg-[#133c1d] px-4 py-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-sm font-sg font-bold text-white">Find Stations</p>
                <p className="text-xs text-[#f5faf0]/80">
                  {pagination ? `${pagination.total} near you` : 'Searching…'}
                </p>
              </div>
              <button onClick={() => setSidebarOpen(false)}
                className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#8cc63f]/20 text-[#8cc63f] hover:bg-[#8cc63f]/40 transition-colors">
                <X className="h-4 w-4" />
              </button>
            </div>
            {/* Search */}
            ...
            <button
              ...
              {activeFilters > 0 && (
                <span className={cn('ml-auto flex h-4 w-4 items-center justify-center rounded-full text-[10px] font-bold',
                  filtersOpen ? 'bg-white text-[#8cc63f]' : 'bg-[#8cc63f] text-[#133c1d]')}>
                  {activeFilters}
                </span>
              )}
            </button>
                  ...
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <div onClick={() => { setIsVerified(isVerified ? undefined : true); setPage(1) }}
                      className={cn('relative h-5 w-9 cursor-pointer rounded-full transition-colors',
                        isVerified ? 'bg-[#8cc63f]' : 'bg-gray-200')}>
        ...
        <div className="relative flex-1">
          {!sidebarOpen && (
            <button onClick={() => setSidebarOpen(true)}
              ...
              Stations
              {pagination && (
                <span className="rounded-full bg-[#8cc63f] px-1.5 py-0.5 text-[10px] text-[#133c1d] font-bold">
                  {pagination.total}
                </span>
              )}
            </button>
